...
static SemaphoreHandle_t g_control_lock = NULL;
static bool g_mqtt_runtime_capabilities_dirty = false;
...
static bool is_operational_capability_param_index(int idx) {
    switch (idx) {
        case IDX_P12:
        case IDX_P20:
        case IDX_P21:
...
static void mark_mqtt_runtime_capabilities_dirty_by_param_index(int idx) {
    if (is_operational_capability_param_index(idx)) {
        g_mqtt_runtime_capabilities_dirty = true;
    }
}

static void flush_mqtt_runtime_capabilities_if_dirty(void) {
    if (!g_mqtt_runtime_capabilities_dirty) {
        return;
    }

    g_mqtt_runtime_capabilities_dirty = false;
...
static void update_display_logic(void) {
    ...
    switch (current_state) {
        ...
        case STATE_MENU_EDIT: {
            ...
            /* ParÃ¢metros somente leitura devem aparecer dinÃ¢micos. */
...
static void clear_error_manual_ack(void) {
    ...
    /* Com P44=0, reconhecer o erro NÃƒO deve religar o sistema nem perifÃ©ricos.
     * Apenas memoriza o estado anterior para o prÃ³ximo comando de partida. */
...
static void ihm_sync_task(void *arg) {
    ...
    ESP_LOGI(TAG, "Handshake MI concluÃ­do.");
...
/* ============================================================
 * LÃ“GICA DE IHM / BOTÃ•ES SIMULADOS
 * ============================================================ */
            ...
            if (!blocked) {
                ...
                if (temp_edit_value >= params[current_param_idx].min_val && temp_edit_value <= max_limit) {
                    if (!(p_id == 20 && temp_edit_value >= params[IDX_P21].value) &&
                        ...
                        mark_mqtt_runtime_capabilities_dirty_by_param_index(current_param_idx);
                    }
                }
            }
        }
...
static void phase_service(void) {
    ...
    if (prewet_active) {
        if (water_shortage) {
            ESP_LOGW(TAG, "P30 interrompido por falta de Ã¡gua: ligando motor imediatamente.");
            finish_prewet_now();
...
static void exaustao_service(void) {
    ...
    if (exaustao_on && !exaustao_restart_pending && !exaustao_exit_pending && exaustao_end_us > 0 && now >= exaustao_end_us) {
        ESP_LOGW(TAG, "Tempo de exaustÃ£o P86 concluÃ­do.");
...
static void request_exaustao_stop(void) {
    ...
    if (motor_running || output_frequency > 0.1f) {
        ...
        pulse_buttons(BTN_BIT_STOP);
        ESP_LOGW(TAG, "Saindo da exaustÃ£o: parando motor para voltar ao pronto.");
    } else {
        exaustao_exit_pending = false;
        exaustao_on = false;
        g_direction = (uint8_t)params[IDX_P51].value;
...
static void dreno_service(void) {
    ...
    if (dreno_status == DRENO_EM_CURSO && p80 == 2 && dreno_auto_off_us > 0 && now >= dreno_auto_off_us) {
        dreno_status = DRENO_IDLE; /* desliga saÃ­da no MI */
        dreno_auto_off_us = 0;
...
static void handle_button_event(button_id_t id, bool long_press) {
    ...
    if ((exaustao_on || exaustao_restart_pending || exaustao_exit_pending) && id != BTN_EXAUSTAO && id != BTN_MAIS && id != BTN_MENOS) {
        ESP_LOGW(TAG, "Comando bloqueado: durante a exaustÃ£o, somente EXAUSTAO e ajuste de frequÃªncia podem atuar.");
        return;
    }
    ...
    if (id == BTN_SET) {
        ...
        if (!set_allowed) {
            ESP_LOGW(TAG, "SET bloqueado: menu sÃ³ pode ser acessado em rdy, em funcionamento normal, ou durante a prÃ³pria ediÃ§Ã£o do menu.");
            return;
        }
    ...
    switch (id) {
        case BTN_CLIMATIZAR:
            if (params[IDX_P82].value == 0 || params[IDX_P85].value == 0) {
                bomba_on = false;
                ESP_LOGW(TAG, "Climatizar indisponÃ­vel: P82=0 ou P85=0.");
            } else {
                bomba_on = true;
            }
            break;
        ...
        case BTN_SWING:
            if (params[IDX_P81].value == 0) {
                ...
                ESP_LOGW(TAG, "Swing sÃ³ pode ser acionado com o motor ligado.");
            } else {
                swing_on = !swing_on;
            }
            break;
        case BTN_EXAUSTAO: {
            ...
            if (!exaustao_on && !exaustao_restart_pending && !exaustao_exit_pending) {
                if (params[IDX_P86].value == 0) {
                    ESP_LOGW(TAG, "ExaustÃ£o desabilitada por P86=0.");
                    break;
                }
                ...
                if (motor_running || output_frequency > 0.1f) {
                    ...
                    exaustao_end_us = 0;
                    pulse_buttons(BTN_BIT_STOP);
                    ESP_LOGW(TAG, "ExaustÃ£o iniciada: parando motor para reiniciar em sentido invertido.");
                } else {
                    g_direction = exaustao_pending_direction;
                    motor_running = true;
                    system_on = true;
                    ...
                    pulse_buttons(BTN_BIT_START);
                    ESP_LOGW(TAG, "ExaustÃ£o iniciada: motor ligado em sentido invertido.");
                }
            } else {
                request_exaustao_stop();
            }
        ...
        case BTN_DRENO: {
            ...
            if (dreno_status != DRENO_IDLE || dreno_post_wait_active) {
                ESP_LOGW(TAG, "Dreno jÃ¡ estÃ¡ em andamento.");
                return;
            }
...
static esp_err_t control_lock(void) {
    if (g_control_lock == NULL) return ESP_OK;
    return xSemaphoreTake(g_control_lock, pdMS_TO_TICKS(250)) == pdTRUE ? ESP_OK : ESP_ERR_TIMEOUT;
}
static void control_unlock(void) {
    if (g_control_lock != NULL) xSemaphoreGive(g_control_lock);
}
static esp_err_t dispatch_button_event(button_id_t id, bool long_press) {
    esp_err_t err = control_lock();
    if (err != ESP_OK) return err;
    handle_button_event(id, long_press);
    control_unlock();
    flush_mqtt_runtime_capabilities_if_dirty();
    return ESP_OK;
}
static app_last_command_status_t g_mqtt_last_command_status = APP_LAST_COMMAND_IDLE;
static char g_mqtt_last_error_code[APP_CODE_MAX_LEN + 1] = {0};
static void mqtt_set_result(command_result_t *result, bool accepted, bool applied, app_last_command_status_t status, const char *code, const char *message) {
    if (result == NULL) return;
    memset(result, 0, sizeof(*result));
    result->accepted = accepted;
    result->applied = applied;
    result->status = status;
    if (code) strncpy(result->code, code, sizeof(result->code) - 1);
    if (message) strncpy(result->message, message, sizeof(result->message) - 1);
    g_mqtt_last_command_status = status;
    if (status == APP_LAST_COMMAND_FAILED && code) strncpy(g_mqtt_last_error_code, code, sizeof(g_mqtt_last_error_code) - 1);
    if (status == APP_LAST_COMMAND_APPLIED) g_mqtt_last_error_code[0] = '\0';
}
static bool mqtt_motor_active(void) { return motor_running || output_frequency > 0.1f; }
static bool mqtt_system_engaged(void) { return system_on || prewet_active || dryrun_active || mqtt_motor_active(); }
static app_drain_mode_t mqtt_map_drain_mode(int v) { if (v == 2) return APP_DRAIN_MODE_TIMED; if (v == 1) return APP_DRAIN_MODE_HYBRID; return APP_DRAIN_MODE_DISABLED; }
static app_pump_logic_mode_t mqtt_map_pump_logic(int v) { if (v == 2) return APP_PUMP_LOGIC_INDEPENDENT; if (v == 1) return APP_PUMP_LOGIC_LINKED; return APP_PUMP_LOGIC_FORCED_OFF; }
static app_water_sensor_mode_t mqtt_map_water_sensor(int v) { if (v == 2) return APP_WATER_SENSOR_MODE_INVERTED; if (v == 1) return APP_WATER_SENSOR_MODE_NORMAL; return APP_WATER_SENSOR_MODE_DISABLED; }
static app_ready_state_t mqtt_get_ready_state(void) {
    if (current_state == STATE_ERROR) return APP_READY_STATE_FAULT;
    if (dreno_status != DRENO_IDLE || dreno_post_wait_active) return APP_READY_STATE_DRAINING;
    if (prewet_active || exaustao_restart_pending || dryrun_restart_pending) return APP_READY_STATE_STARTING;
    if (dryrun_active || exaustao_exit_pending) return APP_READY_STATE_STOPPING;
    if (mqtt_motor_active()) return APP_READY_STATE_RUNNING;
    return APP_READY_STATE_READY;
}
static app_peripheral_state_t mqtt_get_pump_state(void) { if (params[IDX_P82].value == 0 || params[IDX_P85].value == 0) return APP_PERIPHERAL_UNAVAILABLE; return bomba_on ? APP_PERIPHERAL_ON : APP_PERIPHERAL_OFF; }
static app_peripheral_state_t mqtt_get_swing_state(void) { if (params[IDX_P81].value == 0) return APP_PERIPHERAL_UNAVAILABLE; return swing_on ? APP_PERIPHERAL_ON : APP_PERIPHERAL_OFF; }
static app_peripheral_state_t mqtt_get_drain_state(void) { if (params[IDX_P80].value == 0) return APP_PERIPHERAL_UNAVAILABLE; return (dreno_status != DRENO_IDLE || dreno_post_wait_active) ? APP_PERIPHERAL_ON : APP_PERIPHERAL_OFF; }
esp_err_t ihm_mqtt_adapter_get_capabilities(device_capabilities_t *capabilities) {
    ...
    capabilities->f_min_hz = params[IDX_P20].value;
    capabilities->f_max_hz = params[IDX_P21].value;
    capabilities->pump_available = params[IDX_P82].value != 0 && params[IDX_P85].value != 0;
    capabilities->swing_available = params[IDX_P81].value != 0;
    capabilities->drain_available = params[IDX_P80].value != 0;
    capabilities->water_sensor_enabled = params[IDX_P85].value != 0;
    capabilities->drain_mode = mqtt_map_drain_mode(params[IDX_P80].value);
    capabilities->drain_time_sec = params[IDX_P83].value * 60;
    capabilities->drain_return_delay_sec = params[IDX_P84].value * 60;
    capabilities->pump_logic_mode = mqtt_map_pump_logic(params[IDX_P82].value);
    capabilities->water_sensor_mode = mqtt_map_water_sensor(params[IDX_P85].value);
    capabilities->pre_wet_sec = params[IDX_P30].value * 60;
    capabilities->dry_panel_sec = params[IDX_P31].value * 60;
    capabilities->dry_panel_freq_hz = params[IDX_P32].value;
    capabilities->resume_mode = params[IDX_P12].value == 1 ? APP_RESUME_LAST_STATE : APP_RESUME_ALWAYS_OFF;
    capabilities->auto_reset_mode = params[IDX_P44].value == 1 ? APP_AUTO_RESET_ENABLED : APP_AUTO_RESET_DISABLED;
    control_unlock();
    return ESP_OK;
}
esp_err_t ihm_mqtt_adapter_get_state(device_state_t *state) {
    ...
    state->inverter_running = mqtt_motor_active();
    state->freq_current_hz = (int)(output_frequency + 0.5f);
    state->freq_target_hz = clamp_i(target_frequency, params[IDX_P20].value, params[IDX_P21].value);
    state->pump_state = mqtt_get_pump_state();
    state->swing_state = mqtt_get_swing_state();
    state->drain_state = mqtt_get_drain_state();
    state->water_level_state = params[IDX_P85].value == 0 ? APP_WATER_LEVEL_DISABLED : (water_shortage ? APP_WATER_LEVEL_LOW : APP_WATER_LEVEL_OK);
    state->last_seen = time(NULL);
    state->last_command_status = g_mqtt_last_command_status;
    if (current_error_code > 0) snprintf(state->last_error_code, sizeof(state->last_error_code), "E%02d", current_error_code);
    else if (g_mqtt_last_error_code[0] != '\0') strncpy(state->last_error_code, g_mqtt_last_error_code, sizeof(state->last_error_code) - 1);
    state->ready_state = mqtt_get_ready_state();
    control_unlock();
    return ESP_OK;
}
esp_err_t ihm_mqtt_adapter_execute_command(const app_command_t *command, command_result_t *result) {
    ...
    if (command->type == APP_COMMAND_RUN_DRAIN || command->type == APP_COMMAND_STOP_DRAIN) { bool active = dreno_status != DRENO_IDLE || dreno_post_wait_active; if (params[IDX_P80].value == 0) { mqtt_set_result(result,false,false,APP_LAST_COMMAND_FAILED,"drain_unavailable","Dreno desabilitado"); return ESP_ERR_NOT_SUPPORTED; } if (command->type == APP_COMMAND_RUN_DRAIN && active) { mqtt_set_result(result,true,true,APP_LAST_COMMAND_APPLIED,"ok","Dreno ja estava ativo"); return ESP_OK; } if (command->type == APP_COMMAND_STOP_DRAIN && !active) { mqtt_set_result(result,true,true,APP_LAST_COMMAND_APPLIED,"ok","Dreno ja estava parado"); return ESP_OK; } if (command->type == APP_COMMAND_STOP_DRAIN && params[IDX_P80].value != 1) { mqtt_set_result(result,false,false,APP_LAST_COMMAND_FAILED,"drain_stop_unsupported","Modo de dreno nao permite parada remota imediata"); return ESP_ERR_NOT_SUPPORTED; } err = dispatch_button_event(BTN_DRENO, false); mqtt_set_result(result,err==ESP_OK,err==ESP_OK,err==ESP_OK?APP_LAST_COMMAND_APPLIED:APP_LAST_COMMAND_FAILED,err==ESP_OK?"ok":"drain_blocked",err==ESP_OK?"Comando de dreno aplicado":"Nao foi possivel alterar o dreno"); return err; }
    mqtt_set_result(result,false,false,APP_LAST_COMMAND_FAILED,"unsupported_command","Comando nao suportado");
...
static void print_help(void) {
    ...
    printf("MAIS / MENOS    -> ajuste de frequÃªncia ou navegaÃ§Ã£o no menu\n");
    ...
    printf("EXAUSTAO        -> toggle exaustÃ£o\n");
...
static void process_console_command(char *cmd) {
    ...
    if (strcasecmp(cmd, "ONOFF") == 0 || strcasecmp(cmd, "POWER") == 0) {
        dispatch_button_event(BTN_ONOFF, false);
        return;
    }
    if (strcasecmp(cmd, "MAIS") == 0 || strcmp(cmd, "+") == 0) {
        dispatch_button_event(BTN_MAIS, false);
        return;
    }
    if (strcasecmp(cmd, "MENOS") == 0 || strcmp(cmd, "-") == 0) {
        dispatch_button_event(BTN_MENOS, false);
        return;
    }
    if (strcasecmp(cmd, "SET") == 0) {
        dispatch_button_event(BTN_SET, false);
        return;
    }
    if (strcasecmp(cmd, "SETL") == 0 || strcasecmp(cmd, "M") == 0) {
        dispatch_button_event(BTN_SET, true);
        return;
    }
    if (strcasecmp(cmd, "CLIMA") == 0 || strcasecmp(cmd, "CLIMATIZAR") == 0) {
        dispatch_button_event(BTN_CLIMATIZAR, false);
        return;
    }
    if (strcasecmp(cmd, "VENT") == 0 || strcasecmp(cmd, "VENTILAR") == 0) {
        dispatch_button_event(BTN_VENTILAR, false);
        return;
    }
    if (strcasecmp(cmd, "SWING") == 0) {
        dispatch_button_event(BTN_SWING, false);
        return;
    }
    if (strcasecmp(cmd, "EXAUSTAO") == 0 || strcasecmp(cmd, "EX") == 0) {
        dispatch_button_event(BTN_EXAUSTAO, false);
        return;
    }
    if (strcasecmp(cmd, "DRENO") == 0) {
        dispatch_button_event(BTN_DRENO, false);
        return;
    }
    if (strcasecmp(cmd, "RESETWIFI") == 0) {
        dispatch_button_event(BTN_RESET_WIFI, false);
        return;
    }
...
void app_main(void) {
    ...
    g_control_lock = xSemaphoreCreateMutex();
    if (g_control_lock == NULL) {
        ESP_LOGE(TAG, "Falha ao criar mutex de controle MQTT/console");
    }

    ESP_ERROR_CHECK_WITHOUT_ABORT(schedule_manager_init());
    ...
    ESP_ERROR_CHECK_WITHOUT_ABORT(mqtt_app_start());
